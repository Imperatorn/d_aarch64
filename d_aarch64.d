/+
	Helper program to help you set up your computer for aarch64 development.
+/
module d_aarch64;
...
version (X86) pragma(msg, "You might want to try -m64 build instead so curl is more likely to work. But you can try this anyway.");
...
int main(string[] args)
{
  ...
  if (p.status != 0 && p.status != 1)
  {
    ...
    writeln("I suggest downloading ldc2 from here:
  ...
  writeln(
    "Download a toolchain from https://developer.arm.com/downloads/-/arm-gnu-toolchain-downloads");

  writeln("Example: https://developer.arm.com/-/media/Files/downloads/gnu/12.3.rel1/binrel/arm-gnu-toolchain-12.3.rel1-mingw-w64-i686-aarch64-none-linux-gnu.zip");
  ...
  if (ldcVersion.startsWith("1.19"))
  {
    writefln("Your LDC is too old. Please get version 1.20 or newer.\nYou can download it from %s", "https://github.com/ldc-developers/ldc/releases");
    return 0;
  }
  ...
  else version (Windows)
  {
    ...
    static assert(0, "Find the OS string under your NDK/toolchains and let me know");

  writefln("Downloading aarch64 runtimes... (about 50 MB total, may take a while)");
  ...
  void handleChunk(string url, ubyte[] chunk)
  {
    ...
    while (remainingUncompressedData.length > 512)
    {
      ...
      processTar(&tfh, &size, a[],
        ...
        {
          if (
            header.filename.endsWith("libphobos2-ldc.a") ||
          header.filename.endsWith("libphobos2-ldc-debug.a") ||
          header.filename.endsWith("libdruntime-ldc.a") ||
          header.filename.endsWith("libdruntime-ldc-debug.a")
            )
          {
  ...
  doDownload("https://github.com/ldc-developers/ldc/releases/download/v" ~ ldcVersion ~ "/ldc2-" ~ ldcVersion ~ "-linux-aarch64.tar.xz", &handleChunk);
  xzd = XzDecoder.init;
  doDownload("https://github.com/ldc-developers/ldc/releases/download/v" ~ ldcVersion ~ "/ldc2-" ~ ldcVersion ~ "-linux-aarch64.tar.xz", &handleChunk);


  browse("https://developer.arm.com/downloads/-/arm-gnu-toolchain-downloads");

  //doDownload("https://developer.arm.com/-/media/Files/downloads/gnu/12.3.rel1/binrel/arm-gnu-toolchain-12.3.rel1-mingw-w64-i686-aarch64-none-linux-gnu.zip", &handleChunk);
  ...
  auto config = configTemplate
    ...
    .replace("$OS", osString)
    .replace("$LDC_VERSION", ldcVersion)
    .replace("$EXT", osExt);

  std.file.append(ldcConfigFileName, config);

  writeln("Check your ldc2.conf and correct for your path");
  writeln("-gcc=C:/Program Files (x86)/Arm GNU Toolchain aarch64-none-linux-gnu/12.3 rel1/bin/aarch64-none-linux-gnu-gcc.exe");
...
"aarch64-.*-linux-gnu":
{
    ...
    lib-dirs = [
        "$D_ANDROID/ldc2-$LDC_VERSION-linux-aarch64/lib/",
    ];
    rpath = "";
};
...
void doDownload(string url, void delegate(string, ubyte[]) processChunk)
{
  import std.net.curl;

  auto http = HTTP(url);
  http.onReceive = (ubyte[] data) { processChunk(url, data); return data.length; };
  http.perform();
}
...
+/
version (WithLzmaDecoder) struct XzDecoder
{
  /++
...
///
version (WithLzmaDecoder) unittest
{
 ...
 */
  ...
  static if (__traits(compiles, { import iv.balz; }))
    enum arcz_has_balz = true;
  else
    enum arcz_has_balz = false;
  static if (__traits(compiles, { import iv.zopfli; }))
    enum arcz_has_zopfli = true;
  else
    enum arcz_has_zopfli = false;
  ...
  private:
    ...
    private @property Nfo* nfo()
    {
      pragma(inline, true);
      return cast(Nfo*) nfop;
    }
    ...
    static uint readUint(FILE* fl)
    {
      ...
      uint v;
      if (fread(&v, 1, v.sizeof, fl) != v.sizeof)
    ...
    static void readBuf(FILE* fl, void[] buf)
    {
      if (buf.length > 0)
      {
        if (fl is null)
          throw new Exception("cannot read from closed file");
        if (fread(buf.ptr, 1, buf.length, fl) != buf.length)
          throw new Exception("file reading error");
      }
    }
    ...
    static T* xalloc(T, bool clear = true)(uint mem) if (T.sizeof > 0)
    {
      ...
      static if (clear)
      {
        ...
        static if (is(T == struct))
        {
          ...
          foreach (immutable idx; 0 .. mem)
            memcpy(res + idx, &i, T.sizeof);
        }
        debug (arcz_alloc)
        {
          import core.stdc.stdio : printf;
    ...
    static if (arcz_has_balz)
      static void unpackBlockBalz(void* dest, uint destlen, const(void)* src, uint srclen, uint blocksize)
      {
        Unbalz bz;
  ...
  public:
    ...
    @property FileInfo[string] files()
    {
      return (nfop ? nfo.files : null);
    }
    ...
    void openArchive(const(char)[] filename)
    {
      ...
      if (filename.length == 0)
        throw new Exception("cannot open unnamed archive file");
      if (false && filename.length < 2048)
      { // FIXME the alloca fails on win64 for some reason
      ...
      if (fl is null)
        throw new Exception("cannot open archive file '" ~ filename.idup ~ "'");
      char[4] sign;
      bool useBalz;
      readBuf(fl, sign[]);
      if (sign != "CZA2")
      ...
      case 1:
        ...
        break;
      default:
        throw new Exception("invalid version of archive file '" ~ filename.idup ~ "'");
      }
      ...
      if (pkidxsize == 0 || idxsize == 0 || indexofs == 0)
        throw new Exception("invalid archive file '" ~ filename.idup ~ "'");
      // now read index
      ubyte* idxbuf = null;
      scope (exit)
        xfree(idxbuf);
      {
        ...
        scope (exit)
          xfree(pib);
        if (fseek(fl, indexofs, 0) < 0)
          throw new Exception("seek error in archive file '" ~ filename.idup ~ "'");
      ...
      ubyte getUbyte()
      {
        if (idxsize - idxbufpos < ubyte.sizeof)
          throw new Exception("invalid index for archive file '" ~ filename.idup ~ "'");
        return idxbuf[idxbufpos++];
      }

      uint getUint()
      {
        if (idxsize - idxbufpos < uint.sizeof)
          throw new Exception("invalid index for archive file '" ~ filename.idup ~ "'");
        version (BigEndian)
        {
          import core.bitop : bswap;
      ...
      void getBuf(void[] buf)
      {
        if (buf.length > 0)
        {
          ...
          if (idxsize - idxbufpos < buf.length)
            throw new Exception("invalid index for archive file '" ~ filename.idup ~ "'");
      ...
      debug (arcz_dirread)
        printf("chunk size: %u\nchunk count: %u\nlast chunk size:%u\n", nfo.chunkSize, ccount, nfo
            .lastChunkSize);
      if (ccount == 0 || nfo.chunkSize < 1 || nfo.lastChunkSize < 1 || nfo.lastChunkSize > nfo
        .chunkSize)
        throw new Exception("invalid archive file '" ~ filename.idup ~ "'");
      ...
      auto fcount = getUint;
      if (fcount == 0)
        throw new Exception("empty archive file '" ~ filename.idup ~ "'");
      ...
      foreach (immutable _; 0 .. fcount)
      {
        ...
        else
        {
          ...
          if (nameofs >= idxsize || namelen > idxsize || nameofs + namelen > idxsize)
            throw new Exception("invalid index in archive file '" ~ filename.idup ~ "'");
          FileInfo fi;
          ...
          debug (arcz_dirread)
            printf("file size: %u\nfile chunk: %u\noffset in chunk:%u; name: [%.*s]\n", fi.size, fi.chunk, fi.chunkofs, cast(
                uint) fi.name.length, fi.name.ptr);
    ...
    {
      if (nfop)
        return ((name in nfo.files) !is null);
      else
        return false;
    }
  ...
  private:
    static struct LowLevelPackedRO
    {
      ...
      private @property inout(Nfo*) nfo() inout pure nothrow @trusted @nogc
      {
        pragma(inline, true);
        return cast(typeof(return)) nfop;
      }
      ...
      static void decRef(size_t me)
      {
        if (me)
        {
          ...
          if (--zl.rc == 0)
          {
            ...
            if (zl.chunkData !is null)
              free(zl.chunkData);
            version (arcz_use_more_memory)
              if (zl.pkdata !is null)
                free(zl.pkdata);
            Nfo.decRef(zl.nfop);
            free(zl);
            debug (arcz_rc)
      ...
      uint justEnoughMemory() pure const nothrow @safe @nogc
      {
        ...
        {
          return (totalsize < nfo.chunkSize && stofs + totalsize < nfo.chunkSize ? stofs + totalsize
              : nfo.chunkSize);
        }
      }
      ...
      void unpackNextChunk()
      {
        ...
        if (chunk.pksize == nfo.chunkSize)
        {
          ...
          if (fseek(nfo.afl, chunk.ofs, 0) < 0)
            throw new Exception("ARCZ reading error");
          if (fread(chunkData, 1, nfo.chunkSize, nfo.afl) != nfo.chunkSize)
            throw new Exception("ARCZ reading error");
        ...
        else
        {
          ...
          version (arcz_use_more_memory)
          {
            ...
            if (pkdatasize < chunk.pksize)
            {
              ...
              debug (arcz_alloc)
              {
                ...
                printf("reallocated from %u to %u bytes; %p -> %p\n", cast(uint) pkdatasize, cast(
                    uint) chunk.pksize, pkdata, newpk);
              }
          ...
          if (fseek(nfo.afl, chunk.ofs, 0) < 0)
            throw new Exception("ARCZ reading error");
          if (fread(pkd, 1, chunk.pksize, nfo.afl) != chunk.pksize)
            throw new Exception("ARCZ reading error");
      ...
      void syncReadPos()
      {
        if (pos >= totalsize || pos == lastrdpos)
          return;
      ...
      int read(void* buf, uint count)
      {
        ...
        if (cast(long) pos + count > totalsize)
          count = totalsize - pos;
        auto res = count;
        while (count > 0)
        {
  ...
  private:
    ...
    private @property inout(ArzArchive.LowLevelPackedRO)* zl() inout pure nothrow @trusted @nogc
    {
      pragma(inline, true);
      return cast(typeof(return)) zlp;
    }

    private void decRef()
    {
      pragma(inline, true);
      ArzArchive.LowLevelPackedRO.decRef(zlp);
      zlp = 0;
    }
  ...
  public:
    ...
    @property uint size() const pure nothrow @safe @nogc
    {
      pragma(inline, true);
      return (zlp ? zl.totalsize : 0);
    }

    @property uint tell() const pure nothrow @safe @nogc
    {
      pragma(inline, true);
      return (zlp ? zl.pos : 0);
    }
 ...
 */
  ...
  private:
    void writeUint(uint v)
    {
      ...
      if (fwrite(&v, 1, v.sizeof, arcfl) != v.sizeof)
        throw new Exception("write error"); // signature
    }
    ...
    void writeBuf(const(void)[] buf)
    {
      if (buf.length > 0)
      {
        if (arcfl is null)
          throw new Exception("write error");
        if (fwrite(buf.ptr, 1, buf.length, arcfl) != buf.length)
          throw new Exception("write error"); // signature
      }
    }
    ...
    static if (arcz_has_balz)
      long writePackedBalz(const(void)[] upbuf)
      {
        ...
        bz.compress(
          ...
          (buf) { res += buf.length; writeBuf(buf[]); }, // max mode
    ...
    long writePackedZLib(const(void)[] upbuf)
    {
      ...
      zs.avail_in = 0;
      // initialize packer
      if (deflateInit2(&zs, Z_BEST_COMPRESSION, Z_DEFLATED, 15, 9, 0) != Z_OK)
        throw new Exception("can't write packed data");
      scope (exit)
        deflateEnd(&zs);
    ...
    uint writePackedBuf(const(void)[] upbuf)
    {
      ...
      if (res > uint.max)
        throw new Exception("output archive too big");
    ...
    void flushData()
    {
      if (cdpos > 0)
      {
        ...
        if (cdpos == chunkdata.length && ci.pksize >= chunkdata.length)
        {
          ...
          if (fseek(arcfl, pos, 0) < 0)
            throw new Exception("can't seek in output file");
          ...
          version (Posix)
          {
            ...
            if (pos < 0 || pos >= uint.max)
              throw new Exception("output archive too big");
            if (ftruncate(fileno(arcfl), cast(uint) pos) < 0)
              throw new Exception("error truncating output file");
          }
          ci.pksize = cdpos;
        }
        if (cdpos < chunkdata.length)
          lastChunkSize = cast(uint) cdpos;
        cdpos = 0;
        chunks ~= ci;
      }
      else
    ...
    void closeArc()
    {
      ...
      if (cpos < 0 || cpos > uint.max)
        throw new Exception("output archive too big");
      // write packed index
      debug (arcz_writer)
      {
        import core.stdc.stdio : pinrtf;
  ...
  public:
    this(const(char)[] fname, uint chunkSize = 256 * 1024, Compressor acpr = Compressor.ZLib)
    {
      ...
      static if (!arcz_has_balz)
      {
        if (acpr == Compressor.Balz || acpr == Compressor.BalzMax)
          throw new Exception("no Balz support was compiled in ArcZ");
      }
      static if (!arcz_has_zopfli)
      {
      ...
      if (arcfl is null)
        throw new Exception("can't create output file '" ~ fname.idup ~ "'");
      cdpos = 0;
    ...
    // valid after closing
    @property uint chunksWritten() const pure nothrow @safe @nogc
    {
      pragma(inline, true);
      return statChunks;
    }

    @property uint filesWritten() const pure nothrow @safe @nogc
    {
      pragma(inline, true);
      return statFiles;
    }
...
*/
  ...
  private SRes LzmaDec_DecodeReal(CLzmaDec* p, SizeT limit, const(Byte)* bufLimit)
  {
    ...
    do
    {
      ...
      if (code < bound)
      {
        uint symbol;
        range = bound;
        *(prob) = cast(CLzmaProb)(ttt + ((kBitModelTotal - ttt) >> kNumMoveBits));
        prob = probs + Literal;
...

      {
        range -= bound;
        code -= bound;
        *(prob) = cast(CLzmaProb)(ttt - (ttt >> kNumMoveBits));
        ...
        {
          {
            ...
            {
              range = bound;
              *(probLen) = cast(CLzmaProb)(ttt + ((kBitModelTotal - ttt) >> kNumMoveBits));
            ...
            else
            {
              range -= bound;
              code -= bound;
              *(probLen) = cast(CLzmaProb)(ttt - (ttt >> kNumMoveBits));
  ...
  private SRes LzmaDec_DecodeReal2(CLzmaDec* p, SizeT limit, const(Byte)* bufLimit)
  {
    do
    {
      ...
      if (auto sres = LzmaDec_DecodeReal(p, limit2, bufLimit))
        return sres;
...

    {
      ...
      if (code < bound)
      {
        ...
        if (state < kNumLitStates)
        {
          ...
          do
          {
            mixin(GET_BIT_CHECK!("prob + symbol", "symbol"));
          }
          while (symbol < 0x100);
        }
        else
  ...
  {
    CLzmaProps propNew;
    if (auto sres = LzmaProps_Decode(&propNew, props, propsSize))
      return sres;
    if (auto sres = LzmaDec_AllocateProbs2(p, &propNew))
      return sres;
    p.prop = propNew;
    return SRes.OK;
  }
    ...
    ELzmaStatus* status)
  {
    ...
    *status = LZMA_STATUS_NOT_SPECIFIED;
    ...
    p.dic = null;
    p.probs = null;
    if (auto sres = LzmaDec_AllocateProbs(&p, propData, propSize))
      return sres;
    p.dic = dest;
...
*/
  ...
  auto LZMA2_IS_UNCOMPRESSED_STATE(P)(P p)
  {
    return (((p).control & LZMA2_CONTROL_LZMA) == 0);
  }

  auto LZMA2_GET_LZMA_MODE(P)(P p)
  {
    return (((p).control >> 5) & 3);
  }

  auto LZMA2_IS_THERE_PROP(P)(P mode)
  {
    return ((mode) >= 2);
  }

  enum LZMA2_LCLP_MAX = 4;
  auto LZMA2_DIC_SIZE_FROM_PROP(P)(P p)
  {
    return ((cast(UInt32) 2 | ((p) & 1)) << ((p) / 2 + 11));
  }
  ...
  static ELzma2State Lzma2Dec_UpdateState(CLzma2Dec* p, Byte b)
  {
    ...
    case ELzma2State.LZMA2_STATE_UNPACK1:
      ...
      p.unpackSize++;
      return (LZMA2_IS_UNCOMPRESSED_STATE(p)) ? ELzma2State.LZMA2_STATE_DATA
        : ELzma2State.LZMA2_STATE_PACK0;
      ...
      }
      {
        ...
        if (LZMA2_IS_UNCOMPRESSED_STATE(p))
        {
          ...
          *srcLen += srcSizeCur;
          ...
          p.state = (p.unpackSize == 0) ? ELzma2State.LZMA2_STATE_CONTROL
            : ELzma2State.LZMA2_STATE_DATA_CONT;
        }
        else
        {
